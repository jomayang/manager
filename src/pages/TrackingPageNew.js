...
export default function TrackingPageNew() {
  ...
  useEffect(() => {
    const fetchHistories = async () => {
      try {
        ...
        const x = dataHistories.map((hist) => hist.tracking);
        const uniqueChars = x.filter((element, index) => {
          return x.indexOf(element) === index;
        });
        const except = trackings.filter((tr) => !x.includes(tr));
        console.log('histo', uniqueChars, except);
        if (dataHistories) {
          const object = dataHistories.reduce((obj, item) => {
          ...
          console.log('the object => ', object, Object.keys(object).length);
          setHistories(object);
        }
